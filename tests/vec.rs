...
fn it_implements_individual_uniq_on_vec() {
    ...
    assert_eq!(
        vec![
            1, 2, 3, 1, 3, 2, 1, 3, 1, 2, 3, 1, 2, 3, 3, 1, 2, 3, 3, 1, 2, 3, 1, 2, 3, 3, 4, 1, 5,
            4, 6
        ]
        .uniq(vec![3, 5]),
        vec![1, 2, 4, 6]
...
fn it_implements_individual_uniq_on_vec_via() {
    use array_tool::vec::Uniq;
    assert_eq!(
        vec![1.1, 2.6, 3.7, 4.7, 5.4, 6.6]
            .uniq_via(vec![1.5, 2.7, 5.0, 7.1, 9.4], |l: &f64, r: &f64| l.floor()
                == r.floor()),
        vec![3.7, 4.7, 6.6]
    );
    assert_eq!(
        vec![
            1.2, 2.5, 3.4, 1.2, 3.8, 2.9, 1.0, 3.2, 1.2, 2.5, 3.7, 1.7, 2.9, 3.1, 3.5, 1.6, 2.7,
            3.9, 3.1, 1.5, 2.6, 3.8, 1.2, 2.6, 3.7, 3.8, 4.9, 1.0, 5.1, 4.4, 6.6
        ]
...
fn it_can_return_its_own_unique_via() {
    use array_tool::vec::Uniq;
    assert_eq!(
        vec![1.2, 2.5, 1.4, 3.2, 4.8, 3.9, 4.0, 5.2, 6.2]
            .unique_via(|l: &f64, r: &f64| l.floor() == r.floor()),
        vec![1.2, 2.5, 3.2, 4.8, 5.2, 6.2]
    );
    assert_eq!(
        vec![
            1.2, 2.5, 3.4, 1.2, 3.8, 2.9, 1.0, 3.2, 1.2, 2.5, 3.7, 1.7, 2.9, 3.1, 3.5, 1.6, 2.7,
...
fn it_intersects_if() {
    ...
    assert_eq!(
        vec!['a', 'a', 'c', 'e']
            .intersect_if(vec!['A', 'B', 'C'], |l, r| l.eq_ignore_ascii_case(r)),
        vec!['a', 'c']
    );
}
...
fn it_creates_union() {
    ...
    assert_eq!(
        vec![
            1, 2, 3, 1, 3, 2, 1, 3, 1, 2, 3, 1, 2, 3, 3, 1, 2, 3, 3, 1, 2, 3, 1, 2, 3, 3, 4, 1, 4,
            6
        ]
        .union(vec![3, 5, 7, 8, 0]),
